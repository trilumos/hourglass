...
- **Profile / Account + analytics + history (founder, 2026-06-17, NEEDS DESIGN+BUILD).** No profile/account record exists yet. The Focus Score is currently DERIVED from sessions (not stored as an account value). Founder wants: an account that records the focus number, plus **analytics, session history, block history**. Likely: an account/profile table (or stored aggregates), a Profile screen with history list + charts, collected hourglasses/levels later. Design before building (schema + screens).
