...
- **Flow Block "Focus Score" system (founder, 2026-06-16, CONFIRMED).** Flow Block's signature differentiator / reason to be the default.
  - **Focus Score = average of the recent-10 Flow Block session POINTS** (weighted game system, not a flat time average). **Flow-Block-only.** Stays a consistent "current ability" number (rises with good sessions, falls when you give up). Headline on Home. (Suggested next Flow length stays a separate recent-completed-length estimate.)
  - **Per-session points (weighted):** `points = base(chosen) × completion² + overflow × overflowRate`, where `base(L) = L + L²/100` (longer = more per minute, "depth bonus"), `completion = min(actual/chosen,1)` squared (partial completion penalized hard), `overflow` = minutes past chosen, `overflowRate = (1 + chosen/100) × 1.5` (grit reward). Counts only if focused ≥ 2 min. Constants (D=100, exp 2, 1.5×) tunable.
  ...
  - **Collectible hourglasses:** completing a Flow Block plays drain→flip→empty and **collects one hourglass** (running count; SAME art, no new design). Lives in the future Profile; counted from day one (derivable from completed flow blocks).
  - **Roadmap (later phases, recorded — not v1):** Focus **Levels** (score fills the hourglass → flip → *new* hourglass designs/skins) → **progressive difficulty** (higher levels average more blocks with decaying weights) → **streak effects** (broken streaks pull the score down at higher levels) → **honesty/anti-idle checks** → **PC/web versions with site/app/application blocking + monitoring** (the real work-verification). On phone-only v1, work can't be verified; defense = protect-the-block + the score being a private, self-only number.
