...
- **Flow Block "Focus Score" system (founder, 2026-06-16, CONFIRMED).** Flow Block's signature differentiator / reason to be the default.
  - **Focus Score** = the **recent-10 average** of the user's **Flow Block** focused lengths. **Flow-Block-only** (Pomodoro/Custom do NOT build it). Headline number on Home; also sets the **suggested next Flow Block length** (meet-or-beat loop). Replaces/evolves the old "Focus Stamina" (was recent-5) as the user-facing metric.
  - **Give up** option in-session: ending early records the **actual focused length** → pulls the average down (the willpower cost); pushing longer raises it.
