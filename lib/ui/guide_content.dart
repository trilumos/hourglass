...
const kSustain101 = <GuideChapter>[
  ...
  GuideChapter('Sounds & Themes', 'Shape how Sustain sounds and looks — session cues, color themes, and how you own them.', [
    ...
    GuideTopic(
      'Owning a theme',
      'A theme is a one-time thing you own, so we do not rent it to you. There are two ways to get one, and both are permanent.\n\nBuy a theme à la carte — a one-time purchase, yours forever, on any plan or none at all. Or get Pro Lifetime, which includes every premium theme, plus every theme we add later, for good.\n\nPro Monthly and Yearly include every Pro feature but no themes. That is deliberate: a subscription rents access to what the app does, and it would be strange to hand back a look you had grown into just because a renewal lapsed. Buy the themes you love à la carte instead and they stay yours, whatever happens to your subscription.\n\nIf you ever lose access to a theme that is applied — a refund, say — the app quietly returns to Sand but never forgets your choice.',
    ),
  ]),
  GuideChapter('Sustain Pro', 'What Pro unlocks, the three ways to get it, where themes fit, and restoring it.', [
    GuideTopic(
      'What Pro unlocks',
      'Pro opens up the full depth of Sustain. Where the free app shows your Records and consistency, Pro traces your whole focus story over time and gives you more room to work the way you want.\n\nEverything Pro unlocks runs on data the app already keeps on your device — it is the same focus history, shown in more depth.\n\nColor themes are the one thing Pro does not simply unlock. A theme is a one-time thing you own, so only Pro Lifetime includes them — Monthly and Yearly get every feature listed below, but no themes. See "Owning a theme" in the Sounds & Themes chapter.',
      ['Your full analytics: Focus Score and Focus Stamina over time, Average session, and Follow-through', 'Your peak focus window and your personal-bests timeline', 'A detailed PDF Focus Report (raw CSV export stays free)', 'Unlimited, longer pauses mid-session (up to 10 minutes each)', 'Keep going on Pomodoro and Custom — add another block as a session ends', 'Session reuse — start again from a past session\'s setup', 'Focus Stamina sets your Flow length — your starting length learns from your recent blocks', 'Every Pro feature added in future, included', 'Color themes: Pro Lifetime only — not included on Monthly or Yearly'],
    ),
    GuideTopic(
      'The three tiers',
      'Pro comes three ways, all yours to use offline once unlocked. Monthly is the low-barrier way to try Pro. Yearly is the best value and where most people land. Lifetime is a single one-time payment — Pro forever, with no subscription and no renewal, and every Pro feature we add later is included automatically.\n\nThemes work differently, on purpose. A theme is a one-time thing you own, so we do not rent it to you: Lifetime includes every theme forever, while Monthly and Yearly include every Pro feature but no themes. On a subscription you buy any theme you like à la carte — once — and it stays yours even if you cancel.\n\nMonthly and Yearly auto-renew until you cancel, which you can do any time via Manage subscription (it opens Google Play). Lifetime and à la carte theme purchases are one-time and never expire. Optional services we may add later — such as cloud backup — are separate from Pro, including for Lifetime.\n\nPrices are shown live from the store in your local currency, so this guide does not list them — open Settings, Sustain Pro to see current prices.',
    ),
    GuideTopic(
